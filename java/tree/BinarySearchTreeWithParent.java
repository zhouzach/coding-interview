            ...
            if (node.left != null) {
                ...
                while (node.parent != null) { //迭代到叶子结点
                    ...
                    if (node.parent.left == node) { //--------->当前节点为双亲结点的左孩子
                        ...
                        //当遍历到最右下的一个结点时，由于是先序遍历，此时所有结点都已经遍历，本句会一直向上，直到到达根结点，
                        // 即node.parent == null，退出循环
                    }
                }
            }
        ...
        while (node != null) {
            ...
            if (node.right != null) { //右子树不为空时，迭代右子树
                ...
                while (node.parent != null) { //迭代其双亲结点（本while会一直向上迭代结点，循环体内的if语句会判断双亲结点为左孩子的双亲时，才会打印结点）
                    ...
                    if (node.parent.left == node) { //当前节点为双亲结点的左孩子
                        handle(node.parent);        //由于是中序遍历，双亲结点还没有遍历，此时要打印双亲结点
                        ...
                        //当遍历到最右端的一个结点(有可能不是最右下结点)时，本句会一直向上，直到到达根结点，即node.parent == null，退出循环
                    }
                }
            }
    ...
    public void postorderTravel(Node node) {
        while (node != null) {
            ...
            if (node.left != null) {
                ...
                handle(node);       //当左孩子与右孩子都为空时，打印双亲结点
                ...
                while (node.parent != null) { //迭代其双亲结点（本while会一直向上迭代结点，循环体内的if语句会判断双亲结点为左孩子的双亲时，才会打印结点）
                    ...
                    if (node.parent.left == node) { //当前节点为双亲结点的左孩子
                        ...
                        if (node.parent.right != null) { // 双亲节点右孩子不为空
                            ...
                            handle(node.parent);      // 由于是后序遍历，双亲节点右孩子为空，左孩子遍历后，此时要打印双亲结点
                            node = node.parent;       // 继续向上层迭代
                        }

                    } else {                      //当前节点为双亲结点的右孩子
                        handle(node.parent);      //由于是后序遍历，右孩子遍历过，双亲结点还没有遍历过，因此打印双亲结点
                        node = node.parent;       //继续向上层迭代即可
                        //当遍历到最右下的一个结点时，本句会一边打印双亲结点，一边向上迭代，直到到达根结点，即node.parent == null，退出循环
                    }
                }
            }
